...
fn format_run_detail(
    ...
    let has_metrics = llm_metrics.is_some() || run.total_duration_ms.is_some();
    if has_metrics {
        out.push('\n');
        if let (Some(cost), Some(turns), Some(dur_ms)) = (
            llm_metrics.as_ref().and_then(|m| m.total_cost_usd),
            llm_metrics.as_ref().and_then(|m| m.total_turns),
        ...
        if let (Some(inp), Some(out_t)) = (
            llm_metrics.as_ref().and_then(|m| m.total_input_tokens),
            llm_metrics.as_ref().and_then(|m| m.total_output_tokens),
        ) {
            let cache_read = llm_metrics
                .as_ref()
                .and_then(|m| m.total_cache_read_input_tokens)
                .unwrap_or(0);
            let cache_create = llm_metrics
                .as_ref()
        ...
        if let Some(ref model) = llm_metrics.as_ref().and_then(|m| m.model.clone()) {
