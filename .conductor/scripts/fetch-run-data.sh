...
workflow_name=$(echo "$run_output" | sed -n 's/^[[:space:]]*Name:[[:space:]]*\([^[:space:]]*\).*/\1/p' | head -1)
...
status=$(echo "$run_output" | sed -n 's/^[[:space:]]*Status:[[:space:]]*\([^[:space:]]*\).*/\1/p' | head -1)
status=${status:-unknown}
step_count=$(echo "$run_output" | grep -c '^\s*Step ' || true)
