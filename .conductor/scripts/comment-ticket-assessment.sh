...
# Detect verdict — check NOT READY before READY to avoid substring collision
tmp=$(mktemp)
trap 'rm -f "$tmp"' EXIT

if echo "$PRIOR_OUTPUT" | grep -q "NOT READY"; then
  VERDICT="NOT_READY"
  LABEL="needs-work"
  LABEL_COLOR="e4e669"
  ...
  emit_output '[]' "Could not determine verdict from prior output — no READY or NOT READY found"
  exit 0
fi
...
gh label create "$LABEL" --repo "$OWNER/$REPO" --color "$LABEL_COLOR" --description "$LABEL_DESC" 2>/dev/null || true
