-- Migration 058: drop the FK constraint on workflow_run_steps.child_run_id so
-- that workflow-type steps can store a workflow_runs.id (not just agent_runs.id).
-- SQLite cannot alter column constraints in-place; use table-recreation pattern.
BEGIN;
