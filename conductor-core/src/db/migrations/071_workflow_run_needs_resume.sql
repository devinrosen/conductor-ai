...
-- (same as 047_workflow_run_hooks.sql). Must be run with PRAGMA foreign_keys = OFF
-- (handled in Rust via with_foreign_keys_off).
--
-- Columns preserved verbatim from current schema (all ALTER TABLE additions through 066):
...
CREATE TABLE workflow_runs_new (
    ...
    status                            TEXT NOT NULL DEFAULT 'pending'
                                      CHECK (status IN ('pending','running','waiting','completed','failed','cancelled','timed_out','needs_resume')),
