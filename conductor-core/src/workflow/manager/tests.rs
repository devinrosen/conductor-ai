...
fn set_step_status(conn: &rusqlite::Connection, step_id: &str, status: WorkflowStepStatus) {
...
fn test_list_all_waiting_gate_steps_excludes_non_gate_steps() {
    ...
    let step_id =
        crate::workflow::insert_step(&conn, &run.id, "regular-step", "actor", false, 0, 0).unwrap();
    set_step_status(&conn, &step_id, WorkflowStepStatus::Waiting);
...
fn test_get_active_steps_for_runs_groups_by_run_id() {
    ...
    let step1_active =
        crate::workflow::insert_step(&conn, &run1.id, "step-a", "actor", false, 0, 0).unwrap();
    set_step_status(&conn, &step1_active, WorkflowStepStatus::Running);
    let step1_done =
        crate::workflow::insert_step(&conn, &run1.id, "step-b", "actor", false, 1, 0).unwrap();
    set_step_status(&conn, &step1_done, WorkflowStepStatus::Completed);

    // run2: one running step only
    let step2_active =
        crate::workflow::insert_step(&conn, &run2.id, "step-c", "actor", false, 0, 0).unwrap();
    set_step_status(&conn, &step2_active, WorkflowStepStatus::Running);
...
fn test_get_active_steps_for_runs_includes_waiting_steps() {
    ...
    set_step_status(&conn, &waiting_step, WorkflowStepStatus::Waiting);

    // Insert a second step and leave it Pending — should be excluded.
    let _pending_step =
        crate::workflow::insert_step(&conn, &run.id, "step-b", "actor", false, 1, 0).unwrap();
...
fn test_get_steps_for_runs_returns_all_steps_regardless_of_status() {
    ...
    let step1a =
        crate::workflow::insert_step(&conn, &run1.id, "step-a", "actor", false, 0, 0).unwrap();
    set_step_status(&conn, &step1a, WorkflowStepStatus::Running);
    let step1b =
        crate::workflow::insert_step(&conn, &run1.id, "step-b", "actor", false, 1, 0).unwrap();
    set_step_status(&conn, &step1b, WorkflowStepStatus::Completed);
...
fn test_set_workflow_run_iteration() {
    ...
    // Set iteration to 3.
    crate::workflow::set_workflow_run_iteration(&conn, &run.id, 3).unwrap();
    let fetched = crate::workflow::get_workflow_run(&conn, &run.id)
        .unwrap()
        .unwrap();
    ...
    crate::workflow::set_workflow_run_iteration(&conn, &run.id, 0).unwrap();
    let fetched = crate::workflow::get_workflow_run(&conn, &run.id)
        .unwrap()
        .unwrap();
    assert_eq!(fetched.iteration, 0);
...
fn test_validate_single_surfaces_warnings_for_unknown_bot() {
    ...
    wf.body.push(WorkflowNode::Call(CallNode {
        ...
        output: None,
        with: vec![],
        as_identity: Some("unknown-bot".to_string()),
        plugin_dirs: vec![],
        timeout: None,
        max_turns: None,
    }));
...
fn test_update_step_preserves_child_run_id_when_none_passed() {
    ...
    let step_id =
        crate::workflow::insert_step(&conn, &run.id, "step-with-child", "actor", false, 0, 0)
            .unwrap();

    // Link a child_run_id to the step.
    crate::workflow::update_step_child_run_id(&conn, &step_id, "child-run-123").unwrap();
...
fn test_get_step_summaries_for_runs_single_level_running_step() {
    ...
    set_step_status(&conn, &step_id, WorkflowStepStatus::Running);
...
fn test_get_step_summaries_for_runs_child_chain_traversal() {
    ...
    let child = crate::workflow::create_workflow_run_with_targets(
        &conn,
        "child-wf",
        Some("w1"),
        None,
    ...
    let child_step_id =
        crate::workflow::insert_step(&conn, &child.id, "deploy", "actor", false, 0, 0).unwrap();
    set_step_status(&conn, &child_step_id, WorkflowStepStatus::Running);
...
fn test_get_step_summaries_for_runs_deep_chain() {
    ...
    let grandchild = crate::workflow::create_workflow_run_with_targets(
        &conn,
        "leaf-wf",
        Some("w1"),
        None,
    ...
    let step_id =
        crate::workflow::insert_step(&conn, &grandchild.id, "test", "actor", false, 0, 0).unwrap();
    set_step_status(&conn, &step_id, WorkflowStepStatus::Running);
...
fn test_get_step_summaries_for_runs_no_running_step_omitted() {
    ...
    let step_id =
        crate::workflow::insert_step(&conn, &run.id, "done", "actor", false, 0, 0).unwrap();
    set_step_status(&conn, &step_id, WorkflowStepStatus::Completed);
...
fn test_get_step_summaries_for_runs_multiple_roots() {
    ...
    .unwrap();
    let step1_id =
        crate::workflow::insert_step(&conn, &run1.id, "compile", "actor", false, 0, 0).unwrap();
    set_step_status(&conn, &step1_id, WorkflowStepStatus::Running);
    ...
    .unwrap();
    let step2_id =
        crate::workflow::insert_step(&conn, &run2.id, "lint", "actor", false, 0, 0).unwrap();
    set_step_status(&conn, &step2_id, WorkflowStepStatus::Running);
...
fn test_get_step_summaries_for_runs_multiple_roots_with_chains() {
    ...
    let child_a = crate::workflow::create_workflow_run_with_targets(
        &conn,
        "child-wf-a",
        Some("w1"),
        None,
    ...
    .unwrap();
    let step_a_id =
        crate::workflow::insert_step(&conn, &child_a.id, "deploy", "actor", false, 0, 0).unwrap();
    set_step_status(&conn, &step_a_id, WorkflowStepStatus::Running);
    ...
    .unwrap();
    let step_b_id =
        crate::workflow::insert_step(&conn, &root_b.id, "test", "actor", false, 0, 0).unwrap();
    set_step_status(&conn, &step_b_id, WorkflowStepStatus::Running);
...
fn test_cancel_run_fails_on_terminal_state() {
    ...
    let result = crate::workflow::cancel_run(&conn, &run.id, "too late");
...
fn test_cancel_run_idempotent_when_cancelling() {
    ...
    let result = crate::workflow::cancel_run(&conn, &run.id, "duplicate cancel");
    assert!(
        result.is_ok(),
...
fn test_cancel_run_marks_active_steps_failed() {
    ...
    let running_step =
        crate::workflow::insert_step(&conn, &run.id, "step-a", "actor", false, 0, 0).unwrap();
    set_step_status(&conn, &running_step, WorkflowStepStatus::Running);
    let completed_step =
        crate::workflow::insert_step(&conn, &run.id, "step-b", "actor", false, 1, 0).unwrap();
    set_step_status(&conn, &completed_step, WorkflowStepStatus::Completed);
...
fn test_cancel_run_kills_child_agent_subprocess() {
    ...
    crate::workflow::cancel_run(&conn, &run.id, "Cancelled by user").unwrap();
...
fn test_cancel_run_not_found() {
    ...
    let result = crate::workflow::cancel_run(&conn, "nonexistent-id", "reason");
...
fn test_fail_workflow_run_returns_parent_id() {
    ...
    let parent_run = agent_mgr
        ...
        .unwrap();
    let wf_run = crate::workflow::create_workflow_run(
        &conn,
        "wf",
        Some("w1"),
        &parent_run.id,
    ...
    let returned_parent_id =
        crate::workflow::fail_workflow_run(&conn, &wf_run.id, "engine panic").unwrap();
...
fn complete_with_metrics(conn: &rusqlite::Connection, run_id: &str, input: i64, output: i64) {
    crate::workflow::update_workflow_status(conn, run_id, WorkflowRunStatus::Completed, None, None)
        .unwrap();
    crate::workflow::persist_workflow_metrics(
        conn, run_id, input, output, 0, 0, 1, 0.0, 1000, None,
    )
    .unwrap();
}
...
fn test_run_metrics_returns_completed_run_data() {
    ...
    .unwrap();
    crate::workflow::persist_workflow_metrics(&conn, &run.id, 100, 200, 0, 0, 1, 0.0, 5000, None)
        .unwrap();
...
fn test_run_metrics_filters_by_workflow_name() {
    ...
    .unwrap();
    crate::workflow::persist_workflow_metrics(&conn, &run_a.id, 100, 200, 0, 0, 1, 0.0, 1000, None)
        .unwrap();
    ...
    .unwrap();
    crate::workflow::persist_workflow_metrics(&conn, &run_b.id, 999, 999, 0, 0, 1, 0.0, 9999, None)
        .unwrap();
...
fn test_run_metrics_excludes_zero_metric_runs() {
    ...
    .unwrap();
    crate::workflow::persist_workflow_metrics(&conn, &zero_run.id, 0, 0, 0, 0, 0, 0.0, 0, None)
        .unwrap();
...
fn test_run_metrics_includes_partial_nonzero_run() {
    ...
    .unwrap();
    crate::workflow::persist_workflow_metrics(&conn, &run.id, 0, 0, 0, 0, 1, 0.0, 2500, None)
        .unwrap();
...
fn test_step_failure_heatmap_basic_failure_rate() {
    ...
    .unwrap();
    crate::workflow::update_workflow_status(&conn, &run2.id, WorkflowRunStatus::Failed, None, None)
        .unwrap();
...
fn test_step_retry_analytics_mixed() {
    ...
    .enumerate()
    {
        ...
        crate::workflow::update_workflow_status(&conn, &run.id, wf_status, None, None).unwrap();
...
fn test_get_workflow_percentiles_single_run() {
    ...
    .unwrap();
    crate::workflow::persist_workflow_metrics(&conn, &run.id, 100, 200, 0, 0, 1, 0.05, 5000, None)
        .unwrap();
...
fn test_get_workflow_percentiles_excludes_other_workflows() {
    ...
    .unwrap();
    crate::workflow::persist_workflow_metrics(
        &conn, &run_a.id, 100, 200, 0, 0, 1, 0.10, 5000, None,
    )
    .unwrap();
    ...
    .unwrap();
    crate::workflow::persist_workflow_metrics(
        &conn, &run_b.id, 999, 999, 0, 0, 1, 9.99, 99000, None,
    )
    .unwrap();
...
fn test_get_all_pending_gates_returns_waiting_gate() {
    ...
    let step_id =
        crate::workflow::insert_step(&conn, &run.id, "human-review", "gate", false, 0, 0).unwrap();
    crate::workflow::set_step_gate_info(
        &conn,
        &step_id,
        &GateType::HumanApproval,
...
fn test_get_fan_out_items_checked_returns_items_for_correct_run() {
    ...
    let items = crate::workflow::get_fan_out_items_checked(&conn, &run.id, &step_id, None).unwrap();
...
fn test_get_fan_out_items_checked_rejects_step_from_different_run() {
    ...
    let step_id =
        crate::workflow::insert_step(&conn, &run_a.id, "fan-step", "actor", false, 0, 0).unwrap();

    // Querying with run_b's ID must return WorkflowStepNotInRun.
    let err =
        crate::workflow::get_fan_out_items_checked(&conn, &run_b.id, &step_id, None).unwrap_err();
    assert!(
        matches!(
            err,
...
fn test_get_fan_out_items_checked_rejects_nonexistent_step() {
    ...
    let err =
        crate::workflow::get_fan_out_items_checked(&conn, &run.id, "nonexistent-step-id", None)
            .unwrap_err();
    assert!(
        matches!(
            err,
...
fn test_skip_fan_out_items_by_item_ids_marks_subset_skipped() {
    ...
    // Skip only t1 and t3.
    crate::workflow::skip_fan_out_items_by_item_ids(
        &conn,
        &step_id,
    ...
    let items = crate::workflow::get_fan_out_items(&conn, &step_id, None).unwrap();
...
fn test_skip_fan_out_items_by_item_ids_ignores_non_pending() {
    ...
    let fan_item_db_id =
        crate::workflow::insert_fan_out_item(&conn, &step_id, "ticket", "t1", "TICKET-1").unwrap();
    crate::workflow::update_fan_out_item_running(&conn, &fan_item_db_id, "child-run-1").unwrap();

    // Attempting to skip t1 by item_id should not change a running item.
    crate::workflow::skip_fan_out_items_by_item_ids(&conn, &step_id, &["t1".to_string()]).unwrap();
...
fn test_skip_fan_out_items_by_item_ids_empty_list_is_noop() {
    ...
    crate::workflow::skip_fan_out_items_by_item_ids(&conn, &step_id, &[]).unwrap();
...
fn test_get_fan_out_items_with_status_filter() {
    ...
    let id2 =
        crate::workflow::insert_fan_out_item(&conn, &step_id, "ticket", "t2", "TICKET-2").unwrap();
    crate::workflow::insert_fan_out_item(&conn, &step_id, "ticket", "t3", "TICKET-3").unwrap();
    crate::workflow::update_fan_out_item_running(&conn, &id2, "child-run-2").unwrap();

    // None → all items
    let all = crate::workflow::get_fan_out_items(&conn, &step_id, None).unwrap();
    assert_eq!(all.len(), 3);

    // Some("pending") → only the 2 still-pending items
    let pending = crate::workflow::get_fan_out_items(&conn, &step_id, Some("pending")).unwrap();
    assert_eq!(pending.len(), 2);
    assert!(pending.iter().all(|it| it.status == "pending"));

    // Some("running") → only the 1 dispatched item
    let running = crate::workflow::get_fan_out_items(&conn, &step_id, Some("running")).unwrap();
    assert_eq!(running.len(), 1);
    assert_eq!(running[0].item_id, "t2");

    // Some("nonexistent-status") → empty result
    let none_match =
        crate::workflow::get_fan_out_items(&conn, &step_id, Some("completed")).unwrap();
...
fn test_get_fan_out_items_for_steps_groups_by_step_run_id() {
    ...
    let run = create_worktree_run(&conn, "w1");
    let step_a =
        crate::workflow::insert_step(&conn, &run.id, "fan-a", "actor", false, 0, 0).unwrap();
    let step_b =
        crate::workflow::insert_step(&conn, &run.id, "fan-b", "actor", false, 1, 0).unwrap();
    let step_c =
        crate::workflow::insert_step(&conn, &run.id, "fan-c", "actor", false, 2, 0).unwrap();
    crate::workflow::insert_fan_out_item(&conn, &step_a, "ticket", "a1", "TICKET-A1").unwrap();
    crate::workflow::insert_fan_out_item(&conn, &step_a, "ticket", "a2", "TICKET-A2").unwrap();
    crate::workflow::insert_fan_out_item(&conn, &step_b, "ticket", "b1", "TICKET-B1").unwrap();

    // Empty input shortcut — no DB query, empty map.
    let empty = crate::workflow::get_fan_out_items_for_steps(&conn, &[]).unwrap();
    assert!(empty.is_empty());

    // Multi-step query: items grouped by step_run_id; step_c (no items) is absent.
    let map =
        crate::workflow::get_fan_out_items_for_steps(&conn, &[&step_a, &step_b, &step_c]).unwrap();
    assert_eq!(map.len(), 2);
...
fn test_spike_baseline_excludes_sub_workflow_runs() {
    ...
    for _ in 0..10 {
        let parent_agent_id = make_parent_id(&conn, "w1");
        let sub = crate::workflow::create_workflow_run_with_targets(
            &conn,
            "excl-wf",
            Some("w1"),
            None,
...
fn test_reset_running_items_without_child_run() {
    ...
    .unwrap();
    let step_id =
        crate::workflow::insert_step(&conn, &run.id, "foreach:test", "foreach", false, 0, 0)
            .unwrap();

    // Item A: running, no child_run_id — should be reset to pending.
    let item_a =
        crate::workflow::insert_fan_out_item(&conn, &step_id, "ticket", "ticket-a", "a").unwrap();
    conn.execute(
    ...
    let item_b =
        crate::workflow::insert_fan_out_item(&conn, &step_id, "ticket", "ticket-b", "b").unwrap();
    conn.execute(
    ...
    let item_c =
        crate::workflow::insert_fan_out_item(&conn, &step_id, "ticket", "ticket-c", "c").unwrap();

    // Item D: completed — must NOT be changed.
    let item_d =
        crate::workflow::insert_fan_out_item(&conn, &step_id, "ticket", "ticket-d", "d").unwrap();
    conn.execute(
    ...
    let reset_count =
        crate::workflow::reset_running_items_without_child_run(&conn, &step_id).unwrap();
    assert_eq!(
        reset_count, 1,
...
fn test_find_step_by_name_and_iteration_returns_non_completed_step() {
    ...
    let step_id =
        crate::workflow::insert_step(&conn, &run.id, step_name, "foreach", false, 0, 1).unwrap();
    set_step_status(
        &conn,
        &step_id,
...
fn test_find_step_by_name_and_iteration_ignores_completed_steps() {
    ...
    let step_id =
        crate::workflow::insert_step(&conn, &run.id, step_name, "foreach", false, 0, 0).unwrap();
    set_step_status(
        &conn,
        &step_id,
        crate::workflow::WorkflowStepStatus::Completed,
...
fn test_set_dismissed_marks_run_dismissed() {
    ...
    crate::workflow::set_dismissed(&conn, &run.id, true).unwrap();
...
fn test_set_dismissed_can_undismiss() {
    ...
    crate::workflow::set_dismissed(&conn, &run.id, true).unwrap();
    crate::workflow::set_dismissed(&conn, &run.id, false).unwrap();
...
fn test_set_dismissed_unknown_id_is_noop() {
    ...
    assert!(crate::workflow::set_dismissed(&conn, "nonexistent-id", true).is_ok());
}
...
fn get_gate_approval_state_returns_pending_for_missing_step() {
    let conn = setup_db();
    let state = crate::workflow::get_gate_approval_state(&conn, "nonexistent-step-id").unwrap();
    assert!(
        matches!(
            state,
...
fn get_gate_approval_state_returns_approved_after_approval() {
    ...
    let step_id =
        crate::workflow::insert_step(&conn, &run.id, "approval-gate", "gate", false, 0, 0).unwrap();
    crate::workflow::approve_gate(&conn, &step_id, "alice", Some("looks good"), None, None)
        .unwrap();
    let state = crate::workflow::get_gate_approval_state(&conn, &step_id).unwrap();
    assert!(
        matches!(
            state,
...
fn get_gate_approval_state_returns_rejected_after_rejection() {
    ...
    let run = create_worktree_run(&conn, "w1");
    let step_id =
        crate::workflow::insert_step(&conn, &run.id, "review-gate", "gate", false, 0, 0).unwrap();
    crate::workflow::reject_gate(&conn, &step_id, "bob", Some("needs rework")).unwrap();
    let state = crate::workflow::get_gate_approval_state(&conn, &step_id).unwrap();
    assert!(
        matches!(
            state,
